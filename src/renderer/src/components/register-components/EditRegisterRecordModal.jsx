...
const EditRegisterRecordModal = ({ previousData, onClose, editRecordItem }) => {
  const MAX_LENTGH = 30
  const MAX_PRICE = 99999999
  const [customerName, setCustomerName] = useState(previousData.customer_name || '')
  const [loanAmount, setLoanAmount] = useState(previousData.loan_amount || 0)
  const [debtAmount, setDebtAmount] = useState(previousData.debt_amount || 0)
  ...
  return (
    ...
    >
      <div
        className="bg-background text-text z-60 px-5 border-accent border-2 h-auto fixed rounded-lg shadow-gray-600 shadow-sm | top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 | flex flex-col items-center justify-center"
...
export default EditRegisterRecordModal
